...
export class AdminCase2Component {
  ...
  // Image upload
  @ViewChild('image2Selected') image2Selected!: ElementRef;
  case2Selected!: File;

  imageCase2Selected(event: any): void{
    this.case2Selected = event.target.files[0];
  }

  onImageCase2Selected(event: Event): void{
    if(!this.case2Selected){
      this.toastr.warning('No file selected');
      return;
    }
    
    if(this.case2Selected.type !== 'image/webp'){
      this.toastr.error('Image format should be .webp');
      return;
    }

    const maxFileSize = 100 * 1024; // 100kb
    if(this.case2Selected.size > maxFileSize){
    ...
    reader.onload = (res: any)=>{
      ...
      image.onload = ()=>{
        ...
        formData.append('image', this.case2Selected)
        this.AdminCase2Service.imageCase2Upload(formData, this.image2Selected);
        this.toastr.success('File uploaded successfully. Refresh to see changes');

      };
      image.src = res.target.result;
    };
    reader.readAsDataURL(this.case2Selected);
  }
